...
function fixLifeStageValue(orgValue) {
...
function insertCustomer(db, fieldArray, tenantObj, callback) {
  ...
  customer['Life Stage'] = fixLifeStageValue(fieldArray[19].trim());
...
function updateDbWithOnlineCsv(db, tenantObj, finalCallback) {
  ...
  console.log('\nLoading Online Data for ' + tenantObj.name);
                ...
                db.onlinedatas.insert(online)
                        .then(function(savedonline) {
                          ...
                          if(onlineInsertCounter === 10000) {
                            console.log('\nLoaded Online data for: ' + tenantObj.name);
                            finalCallback();
                          }
                          onlineReader.resume();
                        });
